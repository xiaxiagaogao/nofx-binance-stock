...
func GetWithTimeframes(symbol string, timeframes []string, primaryTimeframe string, count int) (*Data, error) {
	// Default multi-timeframe market data path uses Binance unless exchange is specified elsewhere.
	symbol = NormalizeForExchange(symbol, "binance")
...
// Normalize normalizes symbol to the legacy internal default form.
...
// NormalizeForExchange normalizes symbol according to the target exchange canonical form.
// This avoids leaking legacy xyz:* symbols into Binance execution / sync / query paths.
func NormalizeForExchange(symbol, exchange string) string {
	exchange = strings.ToLower(strings.TrimSpace(exchange))
	symbol = strings.ToUpper(strings.TrimSpace(symbol))

	if exchange == "binance" {
		// Binance stock-like perpetuals should use Binance-native symbols such as TSLAUSDT,
		// not the internal xyz:* representation.
		base := symbol
		if strings.HasPrefix(strings.ToLower(base), "xyz:") {
			base = base[4:]
		}
		base = strings.ReplaceAll(base, "_", "")
		base = strings.ReplaceAll(base, "-SWAP", "")
